...
pub struct LoggingConfig {
	...
	/// podup overrides that: when the block also sets a positive `max-size`
	/// without a driver, podup pins `k8s-file`, because only that driver
	/// reads the typed size cap; without `max-size`, with a non-positive
	/// `max-size` (`0`, `-1`), or without a driver, the field stays None
	/// and the host's containers.conf default applies (#1895).

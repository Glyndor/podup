...
/// form left one buildah working container behind every failed
/// `podup build` on Podman 5.7.0, measured on 2026-09-24 (#1910); the
/// only thing in scope is the request line, so this test pins it on
/// the wire.
#[tokio::test]
...
/// exercises. The streamed body is the part most likely to surface a
/// regression here: the buildah working-container leak is the symptom of
/// the absolute-form request line that the streamed build endpoint
/// sees. Pin the bytes regardless of how the body was framed.
#[tokio::test]
